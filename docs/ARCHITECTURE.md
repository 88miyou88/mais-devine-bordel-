...
Le bouton classique conserve son libellé `Suppr.`. Dans Qui boit, la corbeille est volontairement iconique, conserve une cible tactile de 44 × 44 px et se trouve après le bouton `Fin`.
...
Le stockage `mdb-card-audit-v1` conserve les entrées et la session courante. Le schéma 2 ajoute le journal `edits` avant/après, les changements de difficulté et les motifs libres. L’identifiant `mdb-card-audit-installation-v1` est aléatoire et local ; il sert uniquement à dédupliquer de futurs rapports.

L’éditeur de cartes existant est réutilisé par le raccourci `M`. L’événement `mdb:card-edited` transmet l’état avant/après au service d’audit sans dupliquer la logique d’édition.

## Migration Maestro — V0.9.6.1
