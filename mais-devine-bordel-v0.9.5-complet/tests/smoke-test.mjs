...
assert.match(html, /import\(["']\.\/src\/main\.js\?v=095["']\)/);
...
assert.match(responsiveCss, /\.card-delete-button\{[\s\S]*?width:44px[\s\S]*?height:44px/, "La suppression en jeu doit garder une cible tactile de 44 px");
...
assert.match(config, /APP_VERSION\s*=\s*"0\.9\.5"/);
assert.match(config, /APP_CACHE_NAME\s*=\s*"mdb-v0-9-5"/);
...
assert.match(sw, /CACHE_NAME\s*=\s*"mdb-v0-9-5"/);
...
assert.equal((html.match(/\?v=095/g) || []).length >= expectedStylePaths.length + 1, true, "Les ressources critiques doivent être versionnées");
...
console.log("✓ Arborescence, DOM, CSS, manifeste et cache de la V0.9.5 cohérents");
